...
async function issueCredential(deliveryId, stage, userId) {
  ...
  const token = randomToken(); const otp = randomOtp();
...
async function verifyCredential(deliveryId, courierId, { stage, method, value }) {
  ...
  try {
    ...
    const supplied = method === 'OTP' ? hash(String(value)) : hash(typeof value === 'object' ? value.token : String(value));
    const expected = method === 'OTP' ? record.otp_hash : record.credential_hash;
    if (!crypto.timingSafeEqual(Buffer.from(supplied), Buffer.from(expected))) throw Object.assign(new Error('The handover credential is not valid.'), { status: 400 });
